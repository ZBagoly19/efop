...
INIT_LRS = [0.0400] #, 0.0200, 0.0150, 0.0100, 0.0050, 0.0020, 0.0005, 0.0001]
...
for learn_rate in INIT_LRS:
    ...
    for num_of_layers in NUMBERS_OF_LAYERS:
        ...
        for width in WIDTHES_OF_LAYERS:
            FILE_PRE = "8_8_loss_sched_test_Adam_" + str(learn_rate) + "_sched_10_0.9_bn_no" + "_reg_no"
            #if bn:
            ...
            shuffled_testing_data = np.load("my_testing_data_XYO_VAV_better_skale_mom1sec.npy", allow_pickle=True)
            shuffled_training_data = np.load("my_training_data_XYO_VAV_better_skale_mom1sec.npy", allow_pickle=True)
            ...
            def my_train():
                ...
                for epoch in range(consts.EPOCHS):
                    ...
                    if epoch % consts.EPOCH_SAVE_CONST == consts.EPOCH_SAVE_CONST - 1:
                        if 450 < epoch:
                            path = os.path.join(FILE_PRE + '_net_XYO_VAV__{}_{}_{}.pth'.format(WIDTH_OF_LAYERS, num_of_layers, epoch))
                            print(path)
                            torch.save(net, path)
                    #if lets_print:
                        #print(net)
            ...
            my_train()
            np.save(FILE_PRE + '_XYO_VAV__losses_and_val_losses_of_{}_{}.npy'.format(WIDTH_OF_LAYERS, num_of_layers), losses)
